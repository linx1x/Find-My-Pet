...
class unconnectedMap extends Component {
  ...
  clickHandler = (event, viewport) => {
    ...
    this.setState({
      ...
      popuplng: this.state.popuplnglat.concat(lng),
      popuplat: this.state.popuplnglat.concat(lat),
      markerslng: this.state.markerslng.concat(lnglat),
      markerslat: this.state.markers.concat(lnglat),
      displayMarker: true,
      showPopup: true
    });
    console.log("state", this.state);
  ...
  render() {
    ...
    let markerlat = this.state.markers[1];
    console.log("markerplat", markerlat);
    let markerlng = this.state.markers[0];
    console.log("markerlng", markerlng);
    let poplat = this.state.popuplnglat[1];
    console.log("poplat", poplat);
    let poplng = this.state.popuplnglat[0];
      ...
      <div className="MainDiv">
        ...
        >
          ...
          {this.state.displayMarker ? (
            ...
            <Marker latitude={poplat} longitude={poplng}>
          ...
          {this.state.showPopup ? (
            <Popup
              latitude={this.state.popuplnglat[1]}
              longitude={this.state.popuplnglat[0]}
              closeButton={true}
              closeOnClick={false}
