...
class unconnectedMap extends Component {
  ...
  render() {
    ...
    for (let i = 0; i < this.state.markers.length; i + 2) {
      markerlat = this.state.markers[i] + markerlat;
    }
    console.log("markerplat", markerlat);
    let markerlng = this.state.markers[0];
    for (let i = 1; i < this.state.markers.length; i + 2) {
      markerlng = this.state.markers[i] + markerlng;
    }
